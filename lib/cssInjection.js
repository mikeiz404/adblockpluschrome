...
port.on("elemhide.getSelectors", (msg, sender) =>
{
  ...
  let mirageEnabled = mirageconf.isEnabled(sender.page.id);
  ...
  {
    if (trace)
      return {selectors, trace: true, inject: false, mirageEnabled: mirageEnabled};

    return {trace: false, inject: false, mirageEnabled: mirageEnabled};
  }

  return {selectors, trace, inject: true, mirageEnabled: mirageEnabled};
});
